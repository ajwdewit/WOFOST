...
IN_{up} &= P~+~I _{e} ~-~E _{s} ~+~SS _{t} \, /\Delta t-~SR  \subeqn  \\
...
water-limited produc\-tion situation are calculated. Influence from groundwater is assumed. The
purpose is to quantify the crop water requirements for continuous growth with either
drought stress or water excess, and to quantify a possible reduction of the crop transpira\-tion 
...
Difficulty arises when the groundwater rises into the rooted zone. The following proce\-dure is 
then applied:
\begin{itemize}
...
12\-8,..,16384 cm are calculated and put into an AFGEN table. 
...
\begin{equation}
% eq 6.49
Z _{t} \, {\rm \cdot {0.75ex}}\, \theta _{air\, ,\, t} ~=~ RD\, {\rm \cdot {0.75ex}}\, \theta _{\max } ~-~ W _{t} 
\end{equation}
...
\begin{align}
% eq 6.63
\Delta \psi_{i} &=~ (\, 0.5 ~+~ p\, \sqrt{0.15} \, )\Delta \psi ~~~~ p~=~ -1,\, 0,\, 1~~~~ \Rightarrow \\
\Delta \psi_{1} &=~ 0.1127017\,\Delta \psi \nonumber  \\
\Delta \psi_{2} &=~ 0.5\,\Delta \psi \nonumber  \\
\Delta \psi_{3} &=~ 0.8872983\,\Delta \psi
\end{align}
